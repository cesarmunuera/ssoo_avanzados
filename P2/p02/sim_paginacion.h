...
typedef struct
{
	...
	// El campo sig sirve para mantener los marcos libres en una lista 		// enlazada. Almacena el marco del siguiente marco en la lista.
	// La lista es circular. Para llegar al primero, miramos el sig del 	// ultimo. 
...
typedef struct
{
    ...
    int tampag;
    int numpags;
    spagina * tdp;			// TDP -> tabla de paginas
    int lru;               // Sólo para reemplazo LRU
    unsigned reloj;        // Sólo para reemplazo LRU(t)

    // Tabla de marcos (mantenida sólo por SO)
    int nummarcos;
    smarco * tdm;		// TDM -> tabla de marcos
    int listalibres;	// Almacena el numero del ultimo marco de la lista
...
unsigned sim_mmu (ssistema * S, unsigned dir_virtual, char op);
	...
	//El puntero apunta a una estructura que contiene la tabla de 			paginas y contadores de referencia a memoria. El entero es la pagina a la que está accediendo, y la operacion es E/L
